...
extension UINavigationBar {
    ...
    func configureNavigationBarTitle(color: UIColor, font: UIFont) {
        tintColor = UIColor.white
        
        var newAttributes: [String: Any] = [NSForegroundColorAttributeName: color,
                                            NSFontAttributeName: font]
        if let dictionaryAttributes = titleTextAttributes {
            newAttributes = Dictionary<String, Any>.append(toDictionary: dictionaryAttributes, newElements: newAttributes)
        }
        titleTextAttributes = newAttributes
    }
}
